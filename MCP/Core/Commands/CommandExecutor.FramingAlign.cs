...
namespace RevitMCP.Core
{
    ...
    public static class FramingAlignGeometry
    {
            ...
            double maxFaceFeet)
        {
            ...
            // 橫向對齊(Y,Z)：目標必須「跨在樑的軸線上」。沿軸(X)刻意不設限 —— 轉角情形本樑較短、
            // 與對接構材之間留有間隙，設限會讓轉角案例找不到候選。橫向偏開的平行樑則在此被排除。
            if (!(minY - cm <= endPoint.Y && endPoint.Y <= maxY + cm &&
                  minZ - cm <= endPoint.Z && endPoint.Z <= maxZ + cm))
            {
                result.RejectReason = "endpoint outside target Y/Z range";
                return result;
            }
            ...
            // T 接 vs L 轉角：看目標在「垂直本樑」的水平方向有沒有跨到端點兩側。
            ...
            bool through = pprojs.Min() < -tperp && pprojs.Max() > tperp;
