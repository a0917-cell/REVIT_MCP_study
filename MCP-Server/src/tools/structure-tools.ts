...
export const structureTools: Tool[] = [
    ...
    },
    {
        ...
        description: "自動將結構構架（梁）端點延伸或切齊至鄰近結構構架邊緣、鋼柱邊緣、樓板邊緣或牆面。支援單一/多支構架或當前視圖中的所有結構構架。",
        inputSchema: {
            type: "object",
            properties: {
                framingIds: { type: "array", items: { type: "number" }, description: "可選。指定要對齊的結構構架 Element ID 清單。若未指定，則自動對齊當前視圖內的所有結構構架。" },
                targetCategoryTypes: { type: "array", items: { type: "string" }, description: "可選。對齊目標類別清單，預設為 ['framing', 'column', 'floor', 'wall']。" },
                maxDistanceMm: { type: "number", description: "可選。最大搜尋/延伸距離（毫米 mm），預設為 2000.0 mm。" },
