...
def find_target_center_and_face(doc, current_framing, end_point, ray_dir, target_items, max_dist_feet, geom_opt_3d, our_half_w):
    ...
    for item in target_items:
        ...
        t_bmin = DB.XYZ(min(xs), min(ys), min(zs))
        t_bmax = DB.XYZ(max(xs), max(ys), max(zs))

        dbg['scanned'] += 1

        # 橫向對齊(Y,Z)：構材必須「跨在樑的軸線上」= 端點落在其 Y、Z 範圍內(±150mm)。
        # 沿軸(X)不設限 → 允許轉角(樑短、留間隙)也能找到相接構材。橫向偏開的平行樑則排除。
        if not (t_bmin.Y - cm <= end_point.Y <= t_bmax.Y + cm and
                t_bmin.Z - cm <= end_point.Z <= t_bmax.Z + cm):
            continue
        ...
        # T接 vs L轉角判定：看構材在「垂直本樑」水平方向是否跨端點兩側。
